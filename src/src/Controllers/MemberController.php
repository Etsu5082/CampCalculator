...
class MemberController
{
    ...
    {
        try {
            $members = $this->model->getPending();
            Response::success([
                'members' => $members,
                'count' => count($members),
            ]);
        } catch (Exception $e) {
            Response::error('承認待ち会員の取得に失敗しました: ' . $e->getMessage(), 500, 'FETCH_ERROR');
        }
    }
    ...
    public function pendingCount(array $params): void
    {
        try {
            ...
            Response::error('承認待ち件数の取得に失敗しました: ' . $e->getMessage(), 500, 'FETCH_ERROR');
        }
    }
        ...
        if ($ext === 'csv') {
            ...
            while (($row = fgetcsv($handle)) !== false) {
                ...
                $members[] = [
                    ...
                    'status' => $status, // OB/OGの場合は自動的に'ob_og'
